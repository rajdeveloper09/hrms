...
const EmployeeProfileTabsSection = ({
  employeeId = "",
  attendanceData = [],
  salaryData = [],
  bonusData = [],
  ...
  const isCurrentEmployeeItem = (item) => {
    ...
    return ids.includes(currentEmpId);
  };
  ...
  const getActiveData = () => {
    ...
    let data = [...(tabData[activeTab] || [])].filter(isCurrentEmployeeItem);
 ...
 ========================================= */
  ...
  const originalTabData = (tabData[activeTab] || []).filter(isCurrentEmployeeItem);
...
export default EmployeeProfileTabsSection;